...
void Hovercraft::draw(){ //draw the hover craft
	...
	//p_arrow->draw(X[6], X[7], -0.5, PI + atan2(Tar_Pos[1], Tar_Pos[0]) + atan2(X[7],X[6]) , PI / 2, PI/8); //Added arrow in the game, for some reason the arrow is not to accurate, the equation I've done for the yaw might be wrong
	p_arrow->draw(X[6], X[7], -0.5,  PI +PI/16 + atan2(Tar_Pos[1]-X[7], Tar_Pos[0]-X[6]), PI / 2, PI / 8);
...
void Hovercraft::text(){
	if (View == true){
		text_xy("Killcount: ", 0, 30, 30);
		text_xy(count, 220, 30, 30);
		text_xy("Pilot, your mission is to shoot down the white globes 1 by 1", 350, 0, 30);
	...
	if (View == false){
		...
		text_xy("Killcount: ", 0, 30, 30);
		text_xy(count, 220, 30, 30);
		text_xy("Do you need help aiming? ", 0, 0, 20);
		sim_step(0.007);
	}

	if (count == 5){
		text_xy("Can't stop won't stop", 540, 500, 30);
	}

	if (count == 10){
		text_xy("IS THAT ALL YOU CAN DO SOLDIER???", 540, 500, 30);
	}

	if (count == 15){
		text_xy("Do you have a headache yet?", 540, 500, 30);
	}
}
